...
// The Swift Standard Library provides a method called `sorted(by:)` which sorts
...
// method returns a new array of the same type and size as the old one, with
// its elements in the correct sorted order.  The original array is not
// modified by the `sorted(by:)` method.
//
// See also:
// https://developer.apple.com/reference/swift/sequence/1641066-sorted
...
// the first value should appear before or after the second value once the
// values are sorted. The sorting closure needs to return true if
// the first value should appear before the second value, and
// false otherwise.

// This example is sorting an array of String values, and so the sorting closure
// needs to be a function of type `(String, String) -> Bool`.
