...
assert(ov3 == -56)
ov3 = 100
var ov4 = Int8.addWithOverflow(ov3, ov3)
assert(ov4.0 == -56)
assert(ov4.overflow == true)
...
postfix func +++ (vec: inout Vector2D) -> Vector2D {
  vec += vec
  return vec
}

var vec7 = Vector2D(x: 2.0, y: 3.0)
let vec8 = vec7+++
assert(vec8.x == 4.0)
assert(vec8.y == 6.0)
