...
// stored properties, as described in Assigning Constant Properties During Initialization.
//
...
// initialized when the new range is created and cannot be changed thereafter,
// because it is a constant property.
...
// Because rangeOfFourItems is declared as a constant (with the let keyword), it is
// not possible to change its firstValue property, even though firstValue is a variable property.
...
// The same is not true for classes, which are reference types. If you assign an
// instance of a reference type to a constant, you can still change that instance’s
// variable properties.
...
// its initial value might not be retrieved until after instance
// initialization completes.  Constant properties must always
// have a value before initialization completes, and
// therefore cannot be declared as lazy.
...
// initialization is complete.  Lazy properties are also useful when the
// initial value for a property requires complex or computationally
// expensive setup that should not be performed unless or until
// it is needed.
...
// The DataManager class has a stored property called data, which is initialized
// with a new, empty array of String values. Although the rest of its functionality
// is not shown, the purpose of this DataManager class is to manage and provide
// access to this array of String data.

// Part of the functionality of the DataManager class is the ability to import data
// from a file. This functionality is provided by the DataImporter class, which is
// assumed to take a non-trivial amount of time to initialize. This might be because
// a DataImporter instance needs to open a file and read its contents into memory
// when the DataImporter instance is initialized.

// It is possible for a DataManager instance to manage its data without ever importing
// data from a file, so there is no need to create a new DataImporter instance when
// the DataManager itself is created. Instead, it makes more sense to create the
...
assert(manager.importer.fileName == "data.txt")
...
// to store values and references as part of a class instance. In addition to
// properties, you can use instance variables as a backing store for the values
...
// value is accessed in different contexts and simplifies the property’s declaration
// into a single, definitive statement. All information about the property—including
// its name, type, and memory management characteristics—is defined in a single
// location as part of the type's definition.


// -----------------------------------------------------------------------------
//  Computed property - A property whose value is calculated when used.
// -----------------------------------------------------------------------------
...
assert(square.origin.x == 10.0)
assert(square.origin.y == 10.0)
...
// -----------------------------------------------------------------------------
//  Read-only computed property - A computed property that has only a getter.
// -----------------------------------------------------------------------------

// A read-only computed property always returns a value, and can be accessed
...
// variable properties with the var keyword, because their value is not fixed. The
// let keyword is only used for constant properties, to indicate that their values
// cannot be changed once they are set as part of instance initialization.


// The declaration of a read-only property has a short form.
...
// -----------------------------------------------------------------------------
//  Property observer - A code block that monitors, and can react to,
//                      changes in a particular property's value.
// -----------------------------------------------------------------------------
...
class StepCounter {
  var totalSteps: Int = 0 {
    willSet(newTotalSteps) {
      log.append("About to set totalSteps to \(newTotalSteps)")
    }
    didSet {
      if totalSteps > oldValue  {
        log.append("Added \(totalSteps - oldValue) steps")
      }
    }
  }
}
...
// -----------------------------------------------------------------------------
//  Global constant/variable - A constant/variable.......
//  Local constant/variable - A constant/variable.......
// -----------------------------------------------------------------------------

// The capabilities described above for computing and observing properties are also
// available to global variables and local variables.
...
// -----------------------------------------------------------------------------
//  Type property - A property associated with the type itself.
// -----------------------------------------------------------------------------
...
// no matter how many instances of that type you create. These kinds of properties
// are called type properties.
//
// Type properties are useful for defining values that are universal to all instances
// of a particular type, such as a constant property that all instances can use (like
// a static constant in C), or a variable property that stores a value that is global
...
// default value. This is because the type itself does not have an initializer that
...
// override the superclass’s implementation. The example below shows the syntax
...
// properties, but you can also define read-write computed type properties
...
// model a stereo audio level meter. When a channel’s audio level is 0, none of the
// lights for that channel are lit. When the audio level is 10, all of the lights
// for that channel are lit. In this figure, the left channel has a current level
// of 9, and the right channel has a current level of 7:
//
...
// the new currentLevel value in the maxInputLevelForAllChannels type property.
