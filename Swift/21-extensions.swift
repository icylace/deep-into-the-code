...
// A extension can...
// ...work with types for which you do not have access to the original
//    source code (known as retroactive modeling).
...
// ...extend a protocol to provide implementations of its requirements or add
...
// properties, the names of these properties can be appended to a
// floating-point literal value with dot syntax, as a way to use
// that literal value to perform distance conversions.
...
// property multiplies the value by 1_000.00 to convert into a number
// expressed in meters. Similarly, there are 3.28084 feet in a meter,
// and so the ft computed property divides the underlying Double
...
// parameters, or to provide additional initialization options that
// were not included as part of the type’s original implementation.
...
// default values for all of its stored properties and does not define any
// custom initializers, you can call the default initializer and memberwise
...
// as described in Default Initializers. These initializers can be used to
...
// on the provided center point and size value.  The initializer then calls the
// structure’s automatic memberwise initializer init(origin:size:), which
...
// instance itself. Structure and enumeration methods that modify self or
// its properties must mark the instance method as mutating, just like
// mutating methods from an original implementation.
...
assert(746381295[0] == 5)
assert(746381295[1] == 9)
assert(746381295[2] == 2)
assert(746381295[8] == 7)
...
// Specifically, it expresses whether the number is negative, zero,
// or positive.
...
// iterates over those values in turn. For each integer in the array, the
// function considers the kind computed property for that integer, and
// prints an appropriate description.
